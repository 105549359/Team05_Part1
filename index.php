<?php include 'header.inc'; ?>
<?php include 'nav.inc'; ?>

<main>
   <section class="welcome-banner">
        <div class="container">
            <h2>Welcome to CDS Solutions</h2>
            <p class="lead-paragraph">
                We are a dynamic and innovative IT company, dedicated to creating cutting-edge solutions
                and fostering a collaborative work environment. We believe in empowering our employees
                to reach their full potential and are always looking for talented individuals to join our team.
            </p>
        </div>
    </section>


    <div class="welcome-image container">
        <img src="images/Banner (2).png" alt="Welcome Image" style="width:100%; height:auto;">
    </div>

    <section class="why-choose-us">
        <h2>Why Choose CDS Solutions?</h2>
        <div class="benefits-grid">
            <div class="benefit-item">
                <i class="fas fa-chart-line"></i>
                <h3>Career Growth</h3>
                <p>We invest in your development with continuous learning opportunities and clear career paths.</>
            </div>
            <div class="benefit-item">
                <i class="fas fa-users"></i>
                <h3>Collaborative Culture</h3>
                <p>Join a supportive team where ideas are valued and collaboration thrives.</p>
            </div>
            <div class="benefit-item">
                <i class="fas fa-lightbulb"></i>
                <h3>Innovative Projects</h3>
                <p>Work on exciting, cutting-edge projects that challenge and inspire you.</p>
            </div>
            <div class="benefit-item">
                <i class="fas fa-hand-holding-heart"></>
                <h3>Work-Life Balance</h3>
                <p>Enjoy flexible work options and a commitment to your well-being.</p>
            </div>
        </div>
    </section>

    <section class="featured-jobs">
        <h2>Featured Jobs</h2>
        <div class="job-card">
            <h3>Senior Software Engineer</h3>
            <p>Develop and maintain complex software systems.</p>
            <a href="jobs.php#senior-software-engineer" class="apply-button">Learn More & Apply</a>
        </div>
        <div class="job-card">
            <h3>Data Scientist</h3>
            <p>Analyze and interpret complex data sets to drive business decisions.</p>
            <a href="jobs.php#data-scientist" class="apply-button">Learn More & Apply</a>
        </div>
    </section>

    <section class="company-values">
        <h2>Our Core Values</h2>
        <div class="values-grid">
            <div class="value-item">
                <i class="fas fa-rocket"></i>
                <h3>Innovation</h3>
                <p>We encourage creative thinking and continuous improvement.</p>
            </div>
            <div class="value-item">
                <i class="fas fa-handshake"></i>
                <h3>Collaboration</h3>
                <p>We believe in the power of teamwork and mutual respect.</p>
            </div>
            <div class="value-item">
                <i class="fas fa-gem"></i>
                <h3>Excellence</h3>
                <p>We strive for the highest standards in everything we do.</p>
            </div>
        </div>
    </section>


</main>

<?php include 'footer.inc'; ?>
